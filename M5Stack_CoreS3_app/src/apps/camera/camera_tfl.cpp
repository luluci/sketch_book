...
namespace app
{
	...
	camera_tfl::camera_tfl() : style_select_(), obj_title_lbl_(), is_update_(false), is_menu_open_(false)
	{
	}
	bool camera_tfl::init(lv_obj_t *parent)
	{
		//
		parent_ = parent;
		...
		// label setting
		lv_label_set_text(obj_title_lbl_.get(), "Camera: Tensorflow Lite\n\nCamera is Stop.\nTouch: Start/Stop");
	...
	void camera_tfl::on_show()
	{
		// nothing
		is_menu_open_ = false;
	}
	void camera_tfl::on_hide()
	{
		is_update_ = false;
		update();
	}

	void camera_tfl::on_menu_open()
	{
		// メニューを開いたらカメラ停止
		is_update_ = false;
		is_menu_open_ = true;
	...
	void camera_tfl::on_timer(timer_tick_100ms)
	{
		if (is_update_)
		{
			// カメラ画像更新
